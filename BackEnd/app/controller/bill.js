...
class BillController extends Controller {
  ...
  async list() {
    ...
    try {
      ...
      // 计算当月总收入和支出
      let monList = list.filter(item => moment(Number(item.date)).format('YYYY-MM') == date)
      // 累计支出
      let totalExpense = monList.reduce((cur,item) => {
        if (item.pay_type == 1) {
      ...
      // 累计收入
      let totalIncome = monList.reduce((cur,item) => {
        if (item.pay_type == 2) {
          cur += Number(item.amount);
          return cur
        }
